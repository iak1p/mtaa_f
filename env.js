export const LOCAL_HOST = "172.20.10.2";
export const PORT = 4001;
export const ADRESS = "172.20.10.4:4001";
export const EXPO_PUBLIC_SUPABASE_URL =
  "https://churijlloevkfgiwttgy.supabase.co";
export const EXPO_PUBLIC_SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNodXJpamxsb2V2a2ZnaXd0dGd5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDMyNDQ4NDIsImV4cCI6MjA1ODgyMDg0Mn0.BqkbHUTHlZhwW8FKb20K1YP3Gg4HFPvbQkGQVXoFVCg";
export const EXPO_PUBLIC_SUPABASE_URL1 =
  "https://yzfmkyxkcwocoyznoxhb.supabase.co";
export const EXPO_PUBLIC_SUPABASE_ANON_KEY1 =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inl6Zm1reXhrY3dvY295em5veGhiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDU2MDU4NjQsImV4cCI6MjA2MTE4MTg2NH0.KWiby4nZBc7rCsv0JjTnfQ4yFS5VM0h8bGxAL93Aock";
